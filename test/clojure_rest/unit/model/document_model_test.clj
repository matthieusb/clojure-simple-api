...
(deftest test-document-validation-invalid
  (testing "Testing document validation failed with not enough keywords in map"
    (is (thrown? Exception (schema/validate document/document-schema wrong-document-map-not-enough-keywords))))

  (testing "Testing document validation failed with too many keywords in map"
    (is (thrown? Exception (schema/validate document/document-schema wrong-document-map-too-many-keywords))))

  (testing "Testing document validation failed with invalid types in map"
    (is (thrown? Exception (schema/validate document/document-schema wrong-document-map-invalid-types)))))
...
(def wrong-document-rest-in-map-too-many-keywords {:id_document "1" :title "title1" :description "text1" :oneMoreKeyword "wrong"})
...
(deftest test-document-rest-in-validation-valid
  ...
  (let [validation-res (schema/validate document/document-schema-rest-in valid-document-rest-in-map)]
...
(deftest test-document-rest-in-validation-invalid
  (testing "Testing document rest input validation failed with not enough keywords in map"
    (is (thrown? Exception (schema/validate document/document-schema-rest-in wrong-document-rest-in-map-not-enough-keywords)))))

  (testing "Testing document rest input validation failed with too many keywords in map"
    (is (thrown? Exception (schema/validate document/document-schema-rest-in wrong-document-rest-in-map-too-many-keywords))))

  (testing "Testing document rest input validation failed with invalid types in map"
    (is (thrown? Exception (schema/validate document/document-schema-rest-in wrong-document-rest-in-map-invalid-types))))
